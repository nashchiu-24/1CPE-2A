...
int GG(const vector<int>& arr, int target) {
    ...
    searchresults II(const string& name, const function<int()>& algo ) {
    ...
    void HH(int rank, const string& algoname, int result, int comparisons, double duration) {
    cout << rank << ". " << algoname << " - ";
    if (result != -1) {
    ...
    cout << comparisons << " comparisons, " << "time: " << duration << " ms" << endl;
}
...
int main() {
    ...
    sort(results.begin(), results.end(), [](const searchresults& a, const searchresults& b) {
        return a.duration < b.duration;
    });

    cout << "\n--- ranking by time ---\n";
    for (int i = 0; i < results.size(); i++) {
        HH(i + 1, results[i].name, results[i].result, results[i].comparisonscount, results[i].duration);
    };
